...
export default class SimulationService {
    public static async simulate(requestBody: SimulationRequest) {
        return axios.post<Simulation>("http://18.230.61.96:8080/simulation", requestBody)
    }

    public static async saveSimulation(simulation: Simulation) {
        let result;
        await axios.post<Simulation>("http://18.230.61.96:8080/simulation/save", simulation)
            .then(response => {
